 ...
 * Testcase Example:  '["OrderedStream","insert","insert","insert","insert",
 * "insert"]\n' +'[[5],[3,"ccccc"],[1,"aaaaa"],[2,"bbbbb"],[5,"eeeee"],[4,
 * "ddddd"]]'
 *
...
class OrderedStream {
  ...
  constructor(n: number) {
    this._stream = new Array<string>(n);
    this._ptr = 0;
  }
  ...
  insert(idKey: number, value: string): string[] {
    ...
    if (this._ptr === idKey - 1) {
      while (this._stream[this._ptr]) this._ptr += 1;
    }

    return this._stream.slice(idKey - 1, this._ptr);
  }
}
// @lc code=end
